...
if os.path.exists('./token/token.txt'): pass
else: raise FileNotFoundError('Please make a "token.txt" and put ur streamlabs token there.')
with open('./token/token.txt','r') as token:
    token = token.read().strip()
    STREAMLABS_SOCKET_TOKEN = token if token else None
...
def main():
    if STREAMLABS_SOCKET_TOKEN == None:
        print("❌ ERROR: 'token.txt' evaluated as None! Please polulate with your Streamlabs Token!")
        sys.exit(1)
